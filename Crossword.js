class Crossword {
  ...
  /**
   * @desc place the first word from the words array into the grid
   */
  placeFirstWord () {
  ...
  addToGridAndPlacedWords (LETTER_COORDINATES, LETTERS, WORD, placedWord) {
    for (let i = 0; i < LETTER_COORDINATES.length; i++) {
      ...
      // if this is the matching letter, we need to add the new word to its partOfWords array
      if (this.grid[ROW][COL]) {
        this.grid[ROW][COL].partOfWords.push(WORD)
      } else {
